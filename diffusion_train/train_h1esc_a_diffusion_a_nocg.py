...
modelstr = "h1esc_a_diffusion_a_nocg"
seed = 314
...
if __name__ == "__main__":
    ...
    try:
        ...
        net = nn.DataParallel(ScoreNet_a(marginal_prob_std=marginal_prob_std_fn))
        MODELA_PATH = "../train/models/model_h1esc_a_gnorm_rcyc_1d2d.corbest.checkpoint"
        net = nn.DataParallel(ScoreNet_a(marginal_prob_std=marginal_prob_std_fn))
    ...
    while True:
        for sequence, target, target_1d in dataloader:
            ...
            optimizer.zero_grad()
            loss, pred = loss_fn_a(net, torch.Tensor(sequence.float()).transpose(1, 2).cuda(), target_cuda, marginal_prob_std_fn)
