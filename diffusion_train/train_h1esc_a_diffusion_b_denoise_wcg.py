...
from diffusion_modules import ScoreNet_b, SeqEncoder
...
modelstr = "h1esc_a_diffusion_b_denoise_wcg"
seed = 314
...
if __name__ == "__main__":
    ...
    try:
        ...
        seqEncoder_dict = seqEncoder.state_dict()
        ...
        for key in pretrained_dict.keys():
            seq_key = key[7:]
            if seq_key in seqEncoder_dict.keys():
                seqEncoder_dict[seq_key] = pretrained_dict[key]
        seqEncoder.load_state_dict(seqEncoder_dict)
    except:
        print("No pretrained SeqEncoder found.")
        exit(1)

    try:
        net = nn.DataParallel(ScoreNet_b(seqEncoder, marginal_prob_std=marginal_prob_std_fn))
        net.load_state_dict(
    ...
    while True:
        for sequence, target, target_1d in dataloader:
            ...
            optimizer.zero_grad()
            loss = loss_fn(net, torch.Tensor(sequence.float()).transpose(1, 2).cuda(), preprocessed_target_cuda, marginal_prob_std_fn)
            ...
            if i % 500 == 0:
                pred = Euler_Maruyama_sampler(net,
                                                torch.Tensor(sequence[[1]]).float().transpose(1, 2).cuda(),
                                                250, 
