...
def generate_matrix(mat_raw, mat_bal, normmat250, eps, isCg, isLog=True):
    ...
    mat250 = np.nanmean(
        np.nanmean(np.reshape(mat_bal, (250, 4, 250, 4)), axis=3), axis=1
    )
...
class HicSubTarFeatures(Target):
    ...
    def __init__(
        ...
        self.normmat250 = (
            np.reshape(normmat, (250, 4, 250, 4)).mean(axis=1).mean(axis=2)
        )
    ...
    def get_feature_data(self, chrom, start, end):
        ...
        vec_w = (
            self.sub_data.bins().fetch((chrom, start, end))["weight"].astype(np.float32)
        )
        mat_w = np.outer(vec_w, vec_w)
        mat_si_bal = mat_si_raw * mat_w
        mat_st_bal = mat_st_raw * mat_w

        mat_si_logb = generate_matrix(
            mat_si_raw,
            mat_si_bal,
            self.normmat250,
            self.eps,
            isCg=self.cg_sub,
            isLog=self.islog_sub,
        )
        mat_st_logb = generate_matrix(
            mat_st_raw,
            mat_st_bal,
            self.normmat250,
            self.eps,
        ...
        mat_t_logb = generate_matrix(
            mat_t_raw,
            mat_t_bal,
            self.normmat250,
            self.eps,
            isCg=self.cg_tar,
            isLog=self.islog_tar,
        )
...
class HicAutoSubFeatures(Target):
    ...
    def __init__(
        ...
        self.sb_normmat250 = (
            np.reshape(sub_normmat, (250, 4, 250, 4)).mean(axis=1).mean(axis=2)
        )
        self.sb_eps = np.min(self.sb_normmat250)

        self.ssb_normmat250 = (
            self.sb_normmat250 * subsub_prob
        )  # np.reshape(sub_normat * subsub_prob, (250, 4, 250, 4)).mean(axis=1).mean(axis=2)
...
class Net(nn.Module):
    ...
    def forward(self, seq, subdata, mask, ignore_seq, ignore_sub):
        ...
        if not ignore_sub:
            tcur = self.hic_encoder(
                torch.cat([subdata.unsqueeze(1), mask.unsqueeze(1)], 1)
            )
...
if __name__ == "__main__":
    ...
    modelstr = f"h1esc.r1000.p{SUB_DATA_PROB}.AE.wcg"
    ...
    net = nn.DataParallel(Net(seqEncoder=seqPart, hicEncoder=hicPart, decoder=decoder))
    ...
    dataloader = SamplerDataLoader(
        val_sampler, num_workers=val_num_workers, batch_size=val_batch_size
    )
    ...
    sampler.mode = "train"
    dataloader = SamplerDataLoader(
        sampler, num_workers=num_workers, batch_size=batch_size, seed=seed
    )
    ...
    for sequence, target in dataloader:
        ...
        targ250_cuda = subtarg250.float().cuda()
        targ250_invalid = ~torch.isfinite(subtarg250)
        targ250_valid = ~torch.isnan(subtarg250)
        targ250_cuda[targ250_invalid] = 0

        inp250_cuda = targ250_cuda.clone().detach()

        pred = net(
            seq=seq_cuda,
            subdata=inp250_cuda,
            mask=targ250_invalid,
            ignore_seq=ign_seq,
            ignore_sub=ign_sub,
        )

        loss = (
            (pred[:, 0, :, :][targ250_valid] - targ250_cuda[targ250_valid]) ** 2
        ).mean()
        ...
        if i % loss_output == 0:
            print(
                f"{i} iteration, train loss: {np.mean(loss_history[-500:])} time elapsed {time.time() - stime}",
                flush=True,
            )
            out.write(
                f"{i} iteration, train loss: {np.mean(loss_history[-500:])} time elapsed {time.time() - stime}\n"
        ...
        if i % validation_output == 0:
            ...
            if not ignore_sequence:
                torch.save(
                    net.module.seq_encoder.state_dict(),
                    "./models/seqpart." + modelstr + ".checkpoint",
                )

            if not ignore_subdata:
                torch.save(
                    net.module.hic_encoder.state_dict(),
                    "./models/hicpart." + modelstr + ".checkpoint",
                )

            torch.save(
                net.module.decoder.state_dict(),
                "./models/decoderpart."
                + out_decoder_name
                + "."
                + modelstr
        ...
        if i % validation_output == 0:
            ...
            for sequence, subinp250, subtarg250, targ250 in zip(
                ...
                if np.mean(np.isnan(targ250)) < 0.7:
                    ...
                    targ250_invalid = ~torch.isfinite(target250_t)
                    targ250_cuda[targ250_invalid] = 0
                    # pdb.set_trace()
                    pred = net(
                        seq=torch.Tensor(sequence).transpose(1, 2).cuda(),
                        subdata=targ250_cuda,
                        mask=targ250_invalid,
                    ...
                    if t < 10:
                        figshow(pred[0, 0, :, :])
                        plt.savefig(
                            "./png/" + modelstr + ".test" + str(t) + ".pred.png"
                        )
                        ...
                        plt.savefig(
                            "./png/" + modelstr + ".test" + str(t) + ".label.png"
                        )
